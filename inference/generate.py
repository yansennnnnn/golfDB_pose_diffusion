...
from wan.utils.utils import cache_image, cache_video, str2bool, ef, clear_cache
...
def _parse_args():
    ...
    parser.add_argument(
        "--ckpt_dir",
        type=str,
        default="train/models/golfdiffusion",
    ...
    parser.add_argument(
        "--prompt",
        type=str,
        default="A standard golf swing motion on a wide course: firm grip, slightly bent legs, forward body lean, eyes focused on the target. Surrounded by grass and trees, sunlight enhances the calm yet dynamic setting. The swing is precise and fluid, captured in both medium and close-up shots.",
    ...
    if rank == 0:
        ...
        if "t2i" in args.task:
            ...
            cache_video(
                ...
                normalize=True,
                value_range=(-1, 1))
            clip = VideoFileClip(args.save_file).resize((130, 130))
            clip_ori = VideoFileClip(args.video)
            final_clip = concatenate_videoclips([clip_ori, clip], method="compose")

            final_clip.write_videofile("result/output.mp4", codec="libx264", audio=False)
